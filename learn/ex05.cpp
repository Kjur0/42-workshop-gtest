...
TEST(whatever, tests) {
	...
	EXPECT_EQ(1, 2); // This will fail, but the test will continue
	EXPECT_EQ(2, 2); // This will pass

	// `ASSERT` will stop the test if the assertion fails
	ASSERT_EQ(1, 1); // This will pass
	ASSERT_EQ(1, 2); // This will fail, and the test will stop here
...
TEST(whatever, more_tests) {
	...
	EXPECT_TRUE(1 == 2); // This will fail, but the test will continue
